...
class RandomFacesScript(scripts.Script):
        ...
        with gr.Accordion("a1111 tweaks - Random Faces", open=False):
            with gr.Column():
                ...
                pool_display = gr.Textbox(
                    ...
                    value="\n".join([f"{i}: {face}" for i, face in enumerate(self.face_pool)]) if self.face_pool else "Empty",
                    interactive=False,
                    lines=5
                )
    ...
    def before_process(self, p, enabled):
        ...
        if hasattr(p, 'script_args') and len(p.script_args) > FACESWAPLAB_FACE_INDEX:
            ...
            print(f"[Random Faces] Warning: Could not set face checkpoint (script_args length: {len(p.script_args) if hasattr(p, 'script_args') else 0})")
